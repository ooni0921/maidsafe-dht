...
#ifndef BASE_TRI_LOGGER_H_
#define BASE_TRI_LOGGER_H_
// TODO(dan): These locations are incorrect.
#if defined(MAIDSAFE_POSIX)
#define FTLOG /tmp/perpetual_data
#elif defined(MAIDSAFE_WIN32)
#define FTLOG 'C:\perpertual_data'
#endif
...
//  static void set_out_stream(::std::auto_ptr<::std::ostream > ostream_ptr);
  ...
  private:
    ...
    explicit tri_logger_t(tri_logger_t const &);
  ...
  //  see:
  //  1. S. Habdank-Wojewodzki, "C++ Trivial Logger",
  //     Overload 77, Feb 2007, pp.19-23
...
#define TRI_LOG(name) \
  ...
  *trivial_logger::tri_logger().ostream_ptr() << \
  boost::posix_time::to_simple_string(now)<< __FILE__ \
  << " [" << __LINE__ << "]: " << #name \
...
//  time(&rawtime);
//  *trivial_logger::tri_logger().ostream_ptr() <<
//  ctime(&rawtime)<< __FILE__
//  << " [" << __LINE__ << "]: " << #name
//  << " = " << (name) << ::std::endl;} }while(false)  // NOLINT
