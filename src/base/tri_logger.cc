...
namespace trivial_logger {
    ...
    // void tri_logger_t::set_out_stream ( ::std::ostream & ostream )
    // {
        // outstream_helper_ptr_ = ::std::auto_ptr<::std::ostream>
        //  ( new null_stream );
    //     outstream_ = &ostream;
    // }
    ...
    #define MAX(x, y) ( (x) < (y) ? (y) : (x))
        ...
        char_type filename[(MAX(sizeof(tlogfilename),
          sizeof(helper_name))+sizeof(ext))/sizeof(char_type)];
        //  create file name
        size_t const len = detail::createfilename(filename,
          tlogfilename, ext, helper_name);
    ...
    ::std::auto_ptr<tri_logger_t>
      detail::tri_logger_out_ptr(new tri_logger_t());
